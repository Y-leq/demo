...
//添加的代码：
    @GetMapping("/student/mylist")
    @ResponseBody
    public ResponseEntity<List<StudentEntity>> getMyStudentList(HttpServletRequest request) {
        Object currentStudentSnObj = request.getSession().getAttribute("user"); // 假设登录时存储的是学号
        if (currentStudentSnObj == null) {
         // 如果Session中没有学生学号，说明未登录或登录状态丢失
         return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(null);
        }
        String currentStudentSn = currentStudentSnObj.toString();

        // 查询当前登录学生的信息
        List<StudentEntity> myStudentList = (List<StudentEntity>) studentRepository.findBySn(currentStudentSn);

        // 如果查询成功且有结果，返回学生信息；否则，返回空列表或相应错误码
        if (!myStudentList.isEmpty()) {
           return ResponseEntity.ok(myStudentList);
        } else {
            // 实际上，如果学生已登录，这里不应该为空，但出于完整性考虑，可以适当处理
